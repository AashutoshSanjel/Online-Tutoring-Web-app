...
export const jobseekerGetAllApplications = catchAsyncErrors(async (req, res, next) => {
    const { role } = req.user;
    if (role === "Teacher") {
        return next(new ErrorHandler("Teacher not allowed to access this resource.", 400));
    }
    const { _id } = req.user;
...
export const jobSeekerDeleteApplication = catchAsyncErrors(async (req, res, next) => {
    const { role } = req.user;
    if (role === "Teacher") {
        return next(new ErrorHandler("Teacher is not allowed to access this resource!", 400));
    }
    const { id } = req.params;
